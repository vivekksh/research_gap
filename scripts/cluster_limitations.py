...
# scripts/cluster_limitations.py
...
def cluster_limitations(input_file: str, output_file: str, max_clusters: int = 5):
    """
    Clusters limitation sentences into research gaps.
    Conservative by design: suppresses clustering when data is insufficient.
    """
    os.makedirs(os.path.dirname(output_file), exist_ok=True)

    with open(input_file, "r", encoding="utf-8") as f:
        sentences = [s.strip() for s in f if len(s.split()) >= 8]

    if len(sentences) < 2:
        with open(output_file, "w", encoding="utf-8") as f:
            f.write("⚠️ Not enough evidence to identify reliable research gaps.\n")
        return
